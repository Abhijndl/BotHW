...
# ══════════════════════════════════════════════════════════════════════════════════
FIRSTCRY_URLS = [
    "https://www.firstcry.com/hot-wheels/0/0/113?sort=new",   # newest first (best for new-listing detection)
    "https://www.firstcry.com/hot-wheels/0/0/113",            # default sort (catches everything else)
]
...
def scrape_firstcry() -> list[dict]:
    ...
    for url in FIRSTCRY_URLS:
        ...
        if r.status_code != 200 or len(r.text) < 5000:
            print(f"  [FC] {url} → HTTP {r.status_code}, len {len(r.text)} (blocked?)")
            continue

        page = r.text
        # Grab only the main product-grid region to avoid footer "popular product" links.
        grid = page
        m = re.search(r'\(\s*245?\s*Items?\s*\)|id="products"|class="[^"]*list_block', page, re.I)
        # (not strictly needed — the block parser below filters by structure)
...
# ══════════════════════════════════════════════════════════════════════════════════
# SOURCE 2 — Minifygram  (Supabase REST, auto-discovered)
# ══════════════════════════════════════════════════════════════════════════════════
# The site is a Lovable/React SPA. Products live in a Supabase project. We pull the
# site's JS bundle, extract the Supabase URL + anon key, then query the products
# table via PostgREST. This survives redeploys because we discover creds each run.
_SB_URL_RE = re.compile(r'https://([a-z0-9]{16,})\.supabase\.co')
_SB_KEY_RE = re.compile(r'(eyJ[A-Za-z0-9_\-]{20,}\.[A-Za-z0-9_\-]{20,}\.[A-Za-z0-9_\-]{20,})')


def _discover_supabase() -> tuple[str, str] | None:
    """Return (base_url, anon_key) for Minifygram's Supabase, or None."""
    try:
    ...
    # Find JS bundle URLs referenced by the shell
    scripts = re.findall(r'src="([^"]+\.js)"', html_txt)
    scripts = [urljoin("https://minifygram.com/", s) for s in scripts]

    blobs = [html_txt]
    for s in scripts[:8]:
        try:
            blobs.append(http.get(s, headers=COMMON_HEADERS, timeout=TIMEOUT, **_IMPERSONATE).text)
        except Exception:
            pass
    ...
    for b in blobs:
        if proj is None:
            mu = _SB_URL_RE.search(b)
            if mu:
                proj = mu.group(0)
        if key is None:
            mk = _SB_KEY_RE.search(b)
            if mk:
                key = mk.group(1)
        if proj and key:
            break
    ...
    if proj and key:
        print(f"  [MG] discovered Supabase project {proj}")
        return proj, key
    print("  [MG] could not auto-discover Supabase creds")
    return None


def scrape_minifygram() -> list[dict]:
    disc = _discover_supabase()
    if not disc:
        return []
    base, key = disc
    headers = {**COMMON_HEADERS, "apikey": key, "Authorization": f"Bearer {key}",
               "Accept": "application/json"}

    # Try the most likely table/column names. PostgREST returns 200 + JSON array
    # for a valid table, 404/400 otherwise, so we just probe candidates.
    candidates = ["products", "product", "listings", "items", "collectibles"]
    rows = None
    for table in candidates:
        u = f"{base}/rest/v1/{table}?select=*&order=created_at.desc&limit=500"
        try:
            r = http.get(u, headers=headers, timeout=TIMEOUT, **_IMPERSONATE)
            if r.status_code == 200:
                data = r.json()
                if isinstance(data, list) and data:
                    rows = data
                    print(f"  [MG] table '{table}' → {len(rows)} rows")
                    break
        except Exception:
            continue
        # retry without the order clause (created_at may not exist)
        try:
            r = http.get(f"{base}/rest/v1/{table}?select=*&limit=500",
                         headers=headers, timeout=TIMEOUT, **_IMPERSONATE)
            if r.status_code == 200:
                data = r.json()
                if isinstance(data, list) and data:
                    rows = data
                    print(f"  [MG] table '{table}' (no order) → {len(rows)} rows")
                    break
        except Exception:
            continue

    if not rows:
        print("  [MG] no product table reachable")
        return []
    ...
    for row in rows:
        ...
        stockv = g("stock", "in_stock", "available", "quantity", "inventory", "status", "is_available")

        if not (rid and name):
            continue
        nl = str(name).lower()
        if "hot wheel" not in nl and "hotwheels" not in nl and "diecast" not in nl:
            # Minifygram sells many brands; keep only Hot Wheels / diecast.
            brand = str(g("brand", "brand_name") or "").lower()
            if "hot wheel" not in brand and "mattel" not in brand:
                continue
        ...
        if isinstance(stockv, bool):
            in_stock = stockv
        elif s in ("true", "in_stock", "instock", "available", "active", "1", "yes"):
            in_stock = True
        elif s in ("false", "out_of_stock", "sold_out", "soldout", "0", "no", "sold out"):
            in_stock = False
        else:
